...
between domains is handled by a software component residing in a higher
privileged level.
...
be loaded, and only authenticated applications can be executed in the trusted
kernel.
...
Partitioning of the physical memory between the domains is performed by the
...
partitioning model is used, even though two trusted applications are in the
same domain, they don't have access to each other's assets unless permission
has been granted.
...
resources of a less trustworthy domain. Finally, domains can be constructed
...
accord with the security policy. For example, memory may be partitioned in

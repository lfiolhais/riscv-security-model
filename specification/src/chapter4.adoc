...
// [caption="Figure {counter:image}: ", reftext="Figure {image}"]
// [title= "TrustZone system"]

TrustZone defines an isolation execution model enforced by the hart. There
are two domains: *Secure* and *Non-Secure*. In the
...
untrusted kernel/hypervsior.
The secure domain has access to the whole physical memory, but the non-secure
domain only has access to the non-secure memory.
...
a secure monitor executing in M-mode, equivalent to a FW RoT, which is
executing in the secure domain. The secure
monitor is responsible for performing inter-domain context switches, and
for resource sharing across domains. Entering the secure monitor is done
through amninterrupt or through an exception, with the correct cause.
Partitioning of the physical memory between the domain is performed by the
secure monitor. By default, all memory belongs to the secure domain.

NOTE: If in the non-secure domain a U-mode application performs a request to
the secure monitor, the request must be passed through directly to M-mode. This
...
TrustZone can have different partitioning models: static or dynamic. A static
partitioning model is enforced solely by the secure monitor. The dynamic
partitioning model is enforced by the secure monitor and by the Secure
Partition Manager inside the secure domain. When a dynamic partitioning model
is used, even though two trusted applications are in the same domain, they
don't have access to each others assets unless permission has been granted.

Hypervisors/Kernels are free to schedule any event/task within the non-secure
domain. In the secure domain only the trusted OS or the Secure Partition
Manager are allowed to schedule events/tasks. Within each domain, the isolation
policy applied depends on the running hypervisor/kernel, and is applied on top
of the isolation policy provided by the current domain.

Secure domains can be created at any level of the privilege stack. Even though
the previous text describes a system with a secure domain enforced at the
machine level, as is the traditional case, it is also possible for RISC-V
systems to create weaker secure domains at lower privileged levels. The
isolation policy of the secure domain is constructed as a stack of isolation
policies. For secure monitors that execute at a lower privilege level than
machine, the memory is partitioned between the domains in accordance to the
current memory abstraction, i.e., memory may be partitioned in
virtual and guest physical memory and not just physical memory.

// [caption="Figure {counter:image}: ", reftext="Figure {image}"]
// [title= "RISC-V TrustZone system"]
...
| Protect Secure domain from the Non-Secure domain.
Secure domain has access to the memory in the non-secure domain.
| (SR_PMP_001, SR_PMP_002) +
...
| Protect the Secure Monitor from lower privilege levels.
| (SR_PMP_001, SR_PMP_002) OR SR_PMP_004 OR SR_MMU_001

| Non-Secure domain doesn't have access to the assets in the secure domain.
| SR_SUD_004 or SR_MMU_003 OR SR_PMP_005
...
Using the aforementioned requirements one can use the isolation primitives to
create Secure and Non-Secure domains. In a conventional model, the secure and
non-secure domains are split into two equal memory regions where the
the secure and non-secure domain memory region are separated using MSB
of the address. If MSB is high, it points to the secure domain.
Otherwise, it points to the non-secure domain. Other models may exist.
...
In the event a HW RoT is available, an IOPMP, IOMPT, or IOMMU may be used at the hart
boundaries to move the responsibility of isolation enforcement from the secure
monitor to the HW RoT. The previously described secure/non-secure domain isolation
can still be employed. On a domain switch, the execution environment can call
the HW RoT through a dedicated system interrupt to perform the required IOPMP
IOMPT, or IOMMU changes.
...
| Guarantee that devices assigned to non-secure domain
  cannot access resources assigned to secure domains.
|  SR_IOM_005, +
  (SR_IOP_001, SR_IOP_002, SR_IOP_003) +
  OR +
  (SR_IOM_001, SR_IOM_002, SR_IOM_003) +
...
Non-Secure devices can be managed by a secure monitor which can
enforce access rules for Non-Secure applications.
...
For example, external debug can be enabled for non-M-mode non-secure
and secure domains without affecting M-mode (recoverable debug). And an
S-/H-mode secure monitor can enable self-hosted debug for a
non-secure application without affecting other applications/kernels
or S-/H-mode.

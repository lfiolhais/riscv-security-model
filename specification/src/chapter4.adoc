...
// [caption="Figure {counter:image}: ", reftext="Figure {image}"]
// [title= "Domain Isolation at the Machine Level"]

Domain isolation is a technique that confines architectural state across
multiple domains. The isolation is enforced by a set of hardware primitives
and software components, together forming the TCB of the system. Each domain
can define its own set of security policies dictating which software can be
executed and under what execution context, and what resources each domain
has access to. A domain adds to the current RISC-V's privilege level isolation
model by providing an orthogonal isolation mechanism between workloads
executing in lower privilege levels. The domain switching and communication
between domain is handled by a software component residing in a higher
privileged level.

In the typical case there are two domains: *Secure* and *Non-Secure*. In the
secure domain trusted applications execute under a trusted kernel, whereas in
the non-secure domain untrusted applications/kernels execute under an
untrusted kernel/hypervisor. The secure domain has access to the whole
physical memory and all other resources, but the non-secure domain only has
access to the non-secure memory portion of the physical and non-secure
resources.

In the non-secure domain, any kernel/hypervisor can be loaded. The untrusted
kernel/hypervisor, in turn, can load any application according to its security
policies. In the secure domain, only kernels that have been authenticated can
be loaded, and only autenticated applications can be executed in the trusted
kernel.

Switching and managing non-secure/secure domains is performed by
the Root Domain Security Manager (RDSM) executing in M-mode, equivalent to
a FW RoT, which is executing in the secure domain. The secure
monitor is responsible for performing inter-domain context switches, and
for resource sharing across domains. Entering the RDSM is done
through an interrupt or through an exception, with the correct cause.
Partitioning of the physical memory between the domain is performed by the
RDSM. By default, all memory belongs to the secure domain.

NOTE: If in the non-secure domain a U-mode application performs a request to
the RDSM, the request must be passed through directly to M-mode. This
can be achieved by zeroing the correct bit in `medeleg`/`mideleg` to the
appropriate exception/interrupt. Not performing the following action mandates
that the exception handler of the untrusted kernel also be part of the TCB.
Certain architectures may provide a dedicated instruction to call the RDSM.

NOTE: RISC-V doesn't possess a dedicated bit indicating if a hart is in the
secure or non-secure domain. However, the domain of the hart can be inferred
by the state of the isolation primitives. By default, M-mode is always in the
secure domain.

Domain isolation can have different partitioning models: static or dynamic.
A static partitioning model is enforced solely by the RDSM. The
dynamic partitioning model is enforced by the RDSM and by the Supervisor
Domain Security Manager (SDSM) inside the secure domain. When a dynamic
partitioning model is used, even though two trusted applications are in th
e same domain, they don't have access to each others assets unless permission
has been granted.

The previous text describes a high-level overview of a traditional domain
isolation with a narrow definition of what is secure and non-secure. In
practice, often times there is no such clear distinction, or
no indication about which domain is more trustworthy than the other.
Further, unlike previously stated, domains may not be constructed in an
hierarchical fashion such that one or more domains have access to the
resources of a less trustworthy domain. Finally, domains can be contructed
and enforced at the system level with a broader definition of resources that
may or may not encompass harts. With this definition, the concepts that
underpin domain isolation can be expanded such that they make
use of different isolation primitives more suited to the security policies
that need to enforced.

Domains can be created at any level of the privilege stack up to S. Therefore,
the Domain Security Manager (DSM) doesn't have to be confined to M-level or
S-level, or even to the isolation primitives of a hart. The isolation policy of
the domain, enforced by the DSM is constructed as a stack of isolation policies
borrowed by high-privileged security policies. For DSMs that execute at lower
privilege levels, the resources are shared/partitioned between the domains in
accordance to the security policy. For example, memory may be partitioned in
virtual and guest physical memory and not just physical memory.

// [caption="Figure {counter:image}: ", reftext="Figure {image}"]
// [title= "Domain Isolation at a non-M-level"]

==== Isolation model

Due to the vast methods a domain can be created, the isolation model only
describes isolation requirements between domains. It is assumed that the
isolation requirement orthogonal to domains is inherited by the execution
context.
...
| Protect assets from one domain from another domain.
| (SR_PMP_001, SR_PMP_002) +
  OR (SR_MPT_001, SR_MPT_002) +
  OR (SR_SUD_001, SR_SUD_002) +
  OR (SR_PMP_004, SR_PMP_005) +
  OR (SR_MMU_001, SR_MMU_002)

| Protect the DSM from lower privilege levels.
| (SR_PMP_001, SR_PMP_002) OR SR_PMP_004 OR SR_MMU_001
...
| A system level HW RoT is recommended
| SR_ROT_001, SR_ROT_002

|===

In the event a HW RoT is available, an IOPMP, IOMPT, or IOMMU may be used at
the hart boundaries to move the responsibility of isolation enforcement from
the hart to the HW RoT. The previously described domain isolation
can still be employed. On a domain switch, the execution environment can call
the HW RoT through a dedicated system interrupt, or through a mailbox to
perform the required IOPMP IOMPT, or IOMMU changes.
...
| Guarantee that devices assigned to one domain
  cannot access resources assigned to other domains.
|  SR_IOM_005, +
  (SR_IOP_001, SR_IOP_002, SR_IOP_003) +
  OR +
  (SR_IOM_001, SR_IOM_002, SR_IOM_003) +
...
| Allow Debug of non-M-mode software while blocking debug of higher privileged secure domains
| SR_DBG_002

| Allow Self-hosted debug of non-secure software
| SR_DBG_003, SR_DBG_004

|===

For example, external debug can be enabled for non-M-mode domains without
affecting M-mode (recoverable debug). And an S-/H-mode DSM can enable
self-hosted debug for a domain without affecting other applications/kernels
or S-/H-mode.
...
| Protect a domain against monitoring or DOS by other domains
| SR_PMU_001, SR_PMU_002, SR_QOS_001, SR_QOS_002, SR_DOS_001, SR_DOS_002

|===

Prevents using event counters to monitor across domains. Event counters can
be managed by higher privileged software as part of context switching

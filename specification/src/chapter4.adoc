...
See xref:chapter3.adoc#_pmp[PMP] and xref:chapter3.adoc#_smepmp[Smepmp]
...
executed under what execution context, and what resources each domain
...
One set of standards that leverages domain isolation is
Global Platform. https://globalplatform.org/[Global platform]
defines technical standards, interface specifications and programming models,
open source firmware, and certification programs for
...
A single TEE provides security services to the Normal domain. TAs are typically
installed at boot by RoT FW and TEE OS, though Global Platform does also define
protocols for installation of TAs at runtime. System configuration and resource
...
Global Platform describes a high-level overview of a traditional domain
isolation with a specific definition of three domains with varying security
policies. The normal domains is considered to be non-secure, and the
TEE and Root domains are considered secure. In practice, often times there
is no such clear distinction, or no indication about which domain is more
trustworthy than the other. Further, unlike previously stated, domains may
not be constructed in an hierarchical fashion such that one or more domains
have access to the resources of a less trustworthy domain. Finally, domains
can be constructed and enforced at the system level with a broader definition
of resources that may or may not encompass harts. With this definition, the
concepts that underpin domain isolation can be expanded such that they make
...
a Domain Security Manager (DSM) doesn't have to be confined to M-level or
...
inherited by high-privileged security policies. For DSMs that execute at lower
privilege levels, the resources are shared/partitioned between the domains in
accord with the security policy.

This section focuses only on the Global Platform set of standards.
However, more broader cases will be pointed out where necessary.
...
See xref:chapter3.adoc#_supervisor_domains_and_mpt[supervisor domains].
See xref:chapter3.adoc#_pmp[PMP] and xref:chapter3.adoc#_smepmp[Smepmp]
See xref:chapter3.adoc#_spmp[sPMP]
See xref:chapter3.adoc#_mmu[MMU]
See xref:chapter3.adoc#_mtt[MTT]

In a broader definition of Domain Isolation, there are no guarantees that the
root domain has access to the resources of any other domain. The enforced
isolation policy may not allow the root domain, or any higher-order domain, to
access resources of any domain, even if the domain is in a lower rank in the
hierarchical chain.
...
In the event a HW RoT is available and under a broader definition of domain
isolation, an IOPMP, IOMPT, or IOMMU may be used at the hart boundaries to
move the responsibility of isolation enforcement from the hart to the HW RoT.
On a domain switch, the HW RoT can interrupt the harts, or the execution
environment can call the HW RoT through a dedicated system interrupt or
through a mailbox, to perform the required IOPMP, IOMPT, or IOMMU changes. This
kind of domain isolation allows the ability to create domains that feature more
capabilities than harts, e.g., NPUs, FPGAs, GPUs, etc. The latter is
traditionally used when a set of computing elements and/or devices are not
part of the TCB of the system. The most common example is a system which
partitions the secure and non-secure domains between different resource
clusters, e.g., a system contains a secure management engine which only
executes authenticated code, and a non-secure compute cluster with
Application Processors (APs) that can execute any code.

[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "HW RoT Domain Isolation"]
...
xref:chapter3.adoc#_supervisor_domains_and_mpt[supervisor domains]) of Normal, TEE, and
